...
*   **Acceptance Criteria:**
    ...
    *   **Direct Catalog Node Controls & Multi-Source Isolation:** Each imported source catalog node in the tree displays its title, version, active/assigned controls badge, inclusion toggle (`Include All` vs `Include Specific`), structure clone action (`📥 Import Full Structure`), and removal action (`🗑️ Remove Source`). When multiple catalogs or profiles are imported, each source is strictly isolated with its distinct title, version, and controls hierarchy in both top tags and the workbench tree.
