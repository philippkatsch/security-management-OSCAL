...
export function MetadataRolesPartiesPanel({ metadata, readOnly, expandedSections, toggleSection, handleFieldChange }) {
  // --- Roles CRUD ---
  const handleAddRole = () => {
  ...
  return (
    <>
      ...
      <div className={metaStyles['metadata-section']} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderLeft: '4px solid var(--color-primary)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
        ...
        {expandedSections.roles && (
          <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              metadata.roles.map((item, idx) => (
                <div key={idx} style={{ padding: '10px', background: 'var(--color-surface)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-sm)' }}>
                  <div style={{ display: 'flex', gap: '8px', marginBottom: '6px' }}>
                    ...
                    />
                    {!readOnly && (
                      <button type="button" className={styles['btn-delete']} onClick={() => handleRemoveRole(idx)} style={{ padding: '2px 6px' }}>🗑</button>
                    )}
                  </div>
                  <DebouncedInput
            ...
            {!readOnly && (
              <button type="button" className={[styles['btn-secondary'], styles['btn-sm']].filter(Boolean).join(' ')} onClick={handleAddRole}>➕ Add Role</button>
            )}
          </div>
        )}
      </div>
      ...
      <div className={metaStyles['metadata-section']} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderLeft: '4px solid var(--color-accent)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
        ...
        {expandedSections.parties && (
          <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              metadata.parties.map((item, idx) => (
                <div key={idx} style={{ padding: '12px', background: 'var(--color-surface)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-sm)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    ...
                    />
                    {!readOnly && (
                      <button type="button" className={styles['btn-delete']} onClick={() => handleRemoveParty(idx)} style={{ padding: '2px 6px' }}>🗑</button>
                    )}
                  </div>
            ...
            {!readOnly && (
              <button type="button" className={[styles['btn-secondary'], styles['btn-sm']].filter(Boolean).join(' ')} onClick={handleAddParty}>➕ Add Party</button>
            )}
          </div>
        )}
      </div>
      ...
      <div className={metaStyles['metadata-section']} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderLeft: '4px solid var(--color-primary)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
        ...
        {expandedSections.locations && (
          <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              metadata.locations.map((item, idx) => (
                <div key={idx} style={{ padding: '12px', background: 'var(--color-surface)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-sm)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    ...
                    />
                    {!readOnly && (
                      <button type="button" className={styles['btn-delete']} onClick={() => handleRemoveLocation(idx)} style={{ padding: '2px 6px' }}>🗑</button>
                    )}
                  </div>
            ...
            {!readOnly && (
              <button type="button" className={[styles['btn-secondary'], styles['btn-sm']].filter(Boolean).join(' ')} onClick={handleAddLocation}>➕ Add Location</button>
            )}
          </div>
        )}
      </div>
      ...
      <div className={metaStyles['metadata-section']} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderLeft: '4px solid var(--color-accent)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
        <div
          onClick={() => toggleSection('responsibleParties')}
          style={{ padding: '16px 20px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontWeight: '800', fontSize: '14px', color: 'var(--color-text)', borderBottom: expandedSections.responsibleParties ? '1px solid var(--color-border)' : 'none' }}
        >
          <span>Responsible Parties ({metadata['responsible-parties']?.length || 0})</span>
          <span style={{ fontSize: '11px', color: 'var(--color-text-muted)' }}>{expandedSections.responsibleParties ? '▼' : '▶'}</span>
        </div>
        {expandedSections.responsibleParties && (
          <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {(!metadata['responsible-parties'] || metadata['responsible-parties'].length === 0) ? (
              <p style={{ fontStyle: 'italic', color: 'var(--color-text-muted)', fontSize: '13px', margin: 0 }}>No responsible parties defined.</p>
            ) : (
              metadata['responsible-parties'].map((item, idx) => (
                <div key={idx} style={{ padding: '12px', background: 'var(--color-surface)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-sm)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    <select
                      value={item['role-id']}
                      onChange={(e) => handleResponsiblePartyChange(idx, 'role-id', e.target.value)}
                      className="form-input"
                      style={{ flex: 1, height: '28px', fontSize: '12px' }}
                      disabled={readOnly}
                    >
                    ...
                    {!readOnly && (
                      <button type="button" className={styles['btn-delete']} onClick={() => handleRemoveResponsibleParty(idx)} style={{ padding: '2px 6px' }}>🗑</button>
                    )}
                  </div>
            ...
            {!readOnly && (
              <button type="button" className={[styles['btn-secondary'], styles['btn-sm']].filter(Boolean).join(' ')} onClick={handleAddResponsibleParty}>➕ Add Responsible Party</button>
            )}
          </div>
        )}
      </div>
