...
export function RiskLogEditor({ value, isEditMode, onChange }) {
  ...
  if (!isEditMode) {
    if (entries.length === 0) return <div className="read-only-val">No risk log entries</div>;
    return (
      <div className="timeline-container">
        {entries.map((entry, i) => (
          <div key={i} className="timeline-card">
            ...
            <div className="read-only-val" style={{ fontSize: '12px' }}>
              {new Date(entry.start).toLocaleString()} {entry.end ? `- ${new Date(entry.end).toLocaleString()}` : ''}
            </div>
            {entry.description && <div className="read-only-val">{entry.description}</div>}
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div className="oscal-editor-section">
      ...
      <button className="btn-add" onClick={addEntry}>+ Add Log Entry</button>
    </div>
  );
}
