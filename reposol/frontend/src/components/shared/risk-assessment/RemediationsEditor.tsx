...
export function RemediationsEditor({ value, isEditMode, onChange }) {
  ...
  if (!isEditMode) {
    if (remediations.length === 0) return <div className={styles['read-only-val']}>No remediations</div>;
    return (
      <div className={styles['oscal-editor-list']}>
        {remediations.map((rem, i) => (
          <div key={i} className={styles['oscal-editor-item']}>
            ...
            {rem.description && <div className={styles['read-only-val']}>{rem.description}</div>}
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div className={styles['oscal-editor-section']}>
        ...
        {remediations.map((rem, i) => (
          <div key={i} className={styles['oscal-editor-item']}>
            <div className={styles['oscal-editor-item-header']}>
              ...
              <button className={styles['btn-remove']} onClick={() => removeRem(i)}>Remove</button>
            </div>
      ...
      <button className={styles['btn-add']} onClick={addRem}>+ Add Remediation</button>
    </div>
  );
}
