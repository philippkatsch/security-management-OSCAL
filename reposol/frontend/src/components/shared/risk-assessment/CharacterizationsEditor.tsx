...
export function CharacterizationsEditor({ value, isEditMode, onChange }) {
  ...
  if (!isEditMode) {
    if (characterizations.length === 0) return <div className={styles['read-only-val']}>No characterizations</div>;
    return (
            ...
            {char.origin?.actors?.length > 0 && (
              <div className={styles['oscal-editor-sublist']}>
                ...
                {char.origin.actors.map((a, j) => (
                  <div key={j} className={styles['read-only-val']}>{a.type}: {a['actor-uuid']}</div>
                ))}
              </div>
            )}
  ...
  return (
    <div className={styles['oscal-editor-section']}>
        ...
        {characterizations.map((char, i) => (
          <div key={i} className={styles['oscal-editor-item']}>
            <div className={styles['oscal-editor-item-header']}>
              ...
              <button className={styles['btn-remove']} onClick={() => removeChar(i)}>Remove</button>
            </div>
      ...
      <button className={styles['btn-add']} onClick={addChar}>+ Add Characterization</button>
    </div>
  );
}
